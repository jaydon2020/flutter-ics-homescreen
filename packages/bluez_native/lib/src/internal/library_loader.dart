...
//
//   1. `BLUEZ_NC_LIB` environment variable override.
//   2. Bare `dlopen("libbluez_nc.so")` — uses the system loader, which
...
//      look for `libbluez_nc.so` next to it. Fixes ivi-homescreen.
//   4. Newest artifact under `.dart_tool/hooks_runner/shared/bluez_native/`
...
DynamicLibrary loadBluezNc() {
  ...
  throw StateError(
    ...
    'Set BLUEZ_NC_LIB=/path/to/libbluez_nc.so to override.',
  );
}
...
String? _findInHooksRunner(String libName) {
  ...
  for (var i = 0; i < 6; i++) {
    final root = Directory(
      '${dir.path}/.dart_tool/hooks_runner/shared/bluez_native/build',
    );
    if (root.existsSync()) {
      File? newest;
