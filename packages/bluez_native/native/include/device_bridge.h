...
class DeviceBridge {
 public:
  ...
  // These run the blocking D-Bus call on a worker thread so the caller stays
  // non-blocking without relying on sdbus-c++ floating async callbacks.
 ...
 private:
  ...
  // Helper to create and execute a D-Bus method call off the caller thread.
