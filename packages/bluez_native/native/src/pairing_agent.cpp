...
namespace {
template <typename F>
void safe_reply(const char* context, F&& reply) {
  try {
    reply();
    ...
    std::visit(
        [](auto& r) {
          ...
          if constexpr (!std::is_same_v<T, std::monostate>) {
            safe_reply("PairingAgent::unregister_agent", [&] {
              r.returnError(
  ...
  for (auto& [id, req] : pending) {
    std::visit(
        [](auto& r) {
          using T = std::decay_t<decltype(r)>;
          if constexpr (!std::is_same_v<T, std::monostate>) {
            safe_reply("PairingAgent::on_cancel", [&] {
...
void PairingAgent::respond(uint64_t request_id,
  ...
  if (!accepted) {
    // ponytail: returning a D-Bus error here can hit a stale sd_bus_message
    // after BlueZ tears pairing down; drop it and let BlueZ cancel/timeout.
    return;
  }
  ...
  std::visit(
      [&response](auto& r) {
        ...
        if constexpr (std::is_same_v<T, sdbus::Result<std::string>>) {
          safe_reply("PairingAgent::respond", [&] {
