...
void bluez_device_cancel_pairing(void* handle, const char* device_path) {
  try {
    ...
    fprintf(stderr, "bluez_device_cancel_pairing: D-Bus error: %s\n", e.what());
  } catch (const std::exception& e) {
    fprintf(stderr, "bluez_device_cancel_pairing: unexpected error (suppressed): %s\n", e.what());
  } catch (...) {
    fprintf(stderr, "bluez_device_cancel_pairing: unknown error suppressed\n");
  }
}
