...
class BluetoothMediaNotifier extends Notifier<BluetoothMediaState> {
  ...
  Future<void> _playbackCommands = Future<void>.value();
  ...
  void _scheduleInitializeRetry() {
    if (_disposed || _initializeRetryTimer != null) return;
    final delay = Duration(
      seconds: switch (_initializeRetryCount++) {
        0 => 2,
        1 => 4,
        2 => 8,
        3 => 16,
    ...
    _initializeRetryTimer = Timer(delay, () {
      _initializeRetryTimer = null;
      unawaited(_initialize());
    });
  }
  ...
  Future<void> _refreshItem(BluezMediaItem item, [int attempt = 0]) async {
    try {
      await item.refresh();
      ...
      if (!_disposed &&
          attempt < 2 &&
          _player?.objectPath == item.playerPath &&
          _objectSubscriptions.containsKey('item:${item.objectPath}')) {
        await Future<void>.delayed(Duration(seconds: attempt + 1));
        if (!_disposed &&
            _player?.objectPath == item.playerPath &&
            _objectSubscriptions.containsKey('item:${item.objectPath}')) {
          await _refreshItem(item, attempt + 1);
        }
        return;
      }
      _refreshedItemPaths.remove(item.objectPath);
  ...
  Future<void> _listFolderItems(
    ...
    try {
      ...
      if (!_disposed &&
          attempt < 2 &&
          _player?.objectPath == playerPath &&
          _objectSubscriptions.containsKey('folder:${folder.objectPath}')) {
        await Future<void>.delayed(Duration(seconds: attempt + 1));
        if (!_disposed &&
            _player?.objectPath == playerPath &&
            _objectSubscriptions.containsKey('folder:${folder.objectPath}')) {
          await _listFolderItems(folder, playerPath, attempt + 1);
        }
        return;
      }
      _listedFolderPaths.remove(folder.objectPath);
  ...
  void playPause() => _queuePlayback(state.playState != PlayState.playing);

  void play() => _queuePlayback(true);

  void pause() => _queuePlayback(false);

  void _queuePlayback(bool playing) {
    final player = _player;
    if (player == null) return;
    final generation = ++_playbackCommandGeneration;
    _setPlayState(playing ? PlayState.playing : PlayState.paused);
    _playbackCommands = _playbackCommands.then(
      (_) => _setPlayback(player, playing, generation),
    );
  }
  ...
  Future<void> _setPlayback(
    ...
    try {
      ...
      if (_disposed || player.objectPath != _player?.objectPath) return;
      await player.refresh();
      if (generation == _playbackCommandGeneration) _publish(player);
    } catch (error) {
      if (!_disposed &&
          generation == _playbackCommandGeneration &&
          player.objectPath == _player?.objectPath) {
        try {
          await player.refresh();
          _publish(player);
        } catch (refreshError) {
          _logError('Unable to refresh Bluetooth media player', refreshError);
        }
      }
  ...
  void playItem(String objectPath) {
    final client = _client;
    final player = _player;
    if (client == null || player == null || _disposed || _playingItem) return;
    _playingItem = true;
    unawaited(() async {
      try {
