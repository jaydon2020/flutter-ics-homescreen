...
const bluetoothSupportedProfileUuids = <String>[
  '0000110a-0000-1000-8000-00805f9b34fb', // A2DP source
  '0000110e-0000-1000-8000-00805f9b34fb', // AVRCP
  '0000111f-0000-1000-8000-00805f9b34fb', // HFP audio gateway
  '0000112f-0000-1000-8000-00805f9b34fb', // PBAP phonebook server
  '00001132-0000-1000-8000-00805f9b34fb', // MAP message access server
];

const bluetoothSmartphoneClassMask = 0x1ffc;
const bluetoothSmartphoneClass = 0x020c;
...
bool bluetoothSupportsMediaProfile(Iterable<String> uuids) => uuids.any(
      (uuid) => bluetoothSupportedProfileUuids.contains(uuid.toLowerCase()),
    );
...
bool bluetoothDeviceIsSupported(BlueZDevice device) =>
    bluetoothDeviceSupportsMediaProfile(device) ||
    (device.deviceClass & bluetoothSmartphoneClassMask) ==
        bluetoothSmartphoneClass;
...
class BluetoothState {
  ...
  final String? disconnectingAddress;
  ...
  BluetoothState copyWith({
    ...
    String? busyAddress,
    String? disconnectingAddress,
    BluetoothOperation? operation,
    bool clearBusyAddress = false,
    bool clearDisconnectingAddress = false,
    ...
    return BluetoothState(
      ...
      disconnectingAddress: clearBusyAddress || clearDisconnectingAddress
          ? null
          : disconnectingAddress ?? this.disconnectingAddress,
...
class BluetoothNotifier extends Notifier<BluetoothState> {
  ...
  Future<void> connect() => _initialization ??= _initializeBluetooth();
  ...
  Future<bool> pairAndConnect(BlueZDevice device) async {
    ...
    if (!device.paired && !bluetoothDeviceIsSupported(device)) {
      _reportError(
    ...
    try {
      ...
      if (ref.read(appProvider) == AppState.bluetoothScan) {
        await exitScanMode();
        ref.read(appProvider.notifier).replace(AppState.bluetooth);
        leftScanPage = true;
      }
      ...
      if (current?.connected ?? false) {
        _updateState(
          (state) => state.copyWith(
            disconnectingAddress: current!.address,
          ),
        );
        await _disconnectDevice(current!);
        _updateState(
          (state) => state.copyWith(clearDisconnectingAddress: true),
        );
      }
  ...
  void _publishDevices() {
    ...
    // Some smartphones do not advertise their media UUIDs before pairing.
    // Keep unrelated devices out of the UI using profiles or device class.
    _updateState(
      (state) => state.copyWith(
        devices: List.unmodifiable(
          _devices.values.where(
            (device) => device.paired || bluetoothDeviceIsSupported(device),
          ),
        ),
      ),
    );
  ...
  void _publishConnectionSignal() {
    if (state.nativeAvailable != true) return;
    ref
