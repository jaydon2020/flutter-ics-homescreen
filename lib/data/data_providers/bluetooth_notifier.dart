...
class BluetoothState {
  ...
  List<BlueZDevice> get pairedDevices =>
      devices.where((d) => d.paired).toList()..sort((a, b) {
...
class BluetoothNotifier extends StateNotifier<BluetoothState> {
  ...
  Future<void> _initializeBluetooth() async {
    try {
      ...
      // The BlueZ agent is NOT registered at startup. It is registered only
      // when the Scan for New Device page is open (see enterScanMode /
      // exitScanMode), so incoming pairing requests are silently rejected by
      // BlueZ at all other times.
  ...
  Future<void> enterScanMode() async {
    ...
    // TODO: uncomment once bluez_native exposes these setters.
  ...
  Future<void> respondToPairing({
    ...
    switch (request.requestType) {
      ...
      case AgentRequestType.displayPasskey:
        if (!accepted) {
          final device = _deviceForPath(request.devicePath);
          if (device != null) await device.cancelPairing();
        }
      case AgentRequestType.cancel:
      case AgentRequestType.release:
        break;
