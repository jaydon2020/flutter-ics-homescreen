...
class _BluetoothPairingRequestState
  ...
  void initState() {
    super.initState();
    ref.listenManual<BlueZAgentRequest?>(
      bluetoothProvider.select((state) => state.pairingRequest),
      (previous, next) {
        final isInput = next?.requestType == AgentRequestType.requestPinCode ||
            next?.requestType == AgentRequestType.requestPasskey;
        if (next != previous && isInput) _controller.clear();
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isInput = request.requestType == AgentRequestType.requestPinCode ||
        request.requestType == AgentRequestType.requestPasskey;
    final code = _pairingCode(request);
        ...
        child: ColoredBox(
          ...
          child: Center(
            child: BluetoothDialog(
              ...
              cancelLabel: 'Cancel',
              onCancel: () => notifier.respondToPairing(accepted: false),
              confirmLabel: 'Confirm',
              onConfirm: () => notifier.respondToPairing(
                accepted: true,
...
class _PairingCodeInput extends StatelessWidget {
  const _PairingCodeInput({required this.controller, required this.isPasskey});
