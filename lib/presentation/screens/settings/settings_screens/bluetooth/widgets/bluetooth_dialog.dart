...
Future<bool> showBluetoothConfirmationDialog(
...
Future<void> connectBluetoothDevice(
  ...
  final connected = ref
      ...
      .where(
        (candidate) =>
            candidate.connected && candidate.address != device.address,
      )
      .firstOrNull;
  if (connected != null) {
    final confirmed = await showBluetoothConfirmationDialog(
      context,
      title: 'Switch Device?',
      message: 'Disconnect ${bluetoothDeviceName(connected)} and connect to '
          '${bluetoothDeviceName(device)}?',
      confirmLabel: 'Switch',
    );
    if (!confirmed || !context.mounted) return;
  }
  await ref.read(bluetoothProvider.notifier).pairAndConnect(device);
}
...
class BluetoothDialog extends StatelessWidget {
  ...
  /// `null` renders a single confirmation action.
