...
class _BluetoothScanContentState extends ConsumerState<BluetoothScanContent> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(bluetoothProvider.notifier).enterScanMode();
    });
  }

  @override
  void dispose() {
    // exitScanMode unregisters the agent and stops discovery regardless of
    // how the page is left (back gesture, button, or programmatic navigation).
    ref.read(bluetoothProvider.notifier).exitScanMode();
    super.dispose();
  }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const Text(
            'No new devices found within 2 minutes.',
            style: TextStyle(
              color: AGLDemoColors.periwinkleColor,
              fontSize: 26,
            ),
